...
QList<QVariant> PasswordManager::searchRecords(const QString &searchTerm)
{
    m_model.clearModel();
    QSqlQuery searchQuery(db);
    searchQuery.prepare("SELECT id, url, login, password FROM passwords WHERE url LIKE :searchTerm OR login LIKE :searchTerm");
    searchQuery.bindValue(":searchTerm", "%" + searchTerm + "%");
    searchQuery.exec();
    return m_model.setQuery(searchQuery);
}
